...
export function useSwipeLimits() {
	...
	const loadCounts = async () => {
		...
		if (!tg?.CloudStorage || ver < 6.9) {
			console.warn('Telegram Cloud Storage not available - skipping limits')
			return
		}
		...
		return new Promise<void>(resolve => {
			try {
				...
				console.warn('CloudStorage not available:', e)
				resolve()
			}
		})
	}
	...
	const incrementLikes = () => {
		...
		saveCounts(swipesCount, newCount)
