...
export default function HomeScreen() {
	...
	const [lumiText, setLumiText] = useState('Привет! Я Луми ✨ Как ты сегодня?')
	...
	// ответ Луми в баббле → затем переход во флоу
	const REPLIES: Record<string, string> = {
		social: 'Ищу тебе компанию на кофе ☕',
		romance: 'О, романтика! Подберу места для свидания 💕',
		inspiration: 'Лови вдохновение — собираю красоту ✨',
		recharge: 'Тогда что-то спокойное, выдохни 🌙',
		social2: 'Найдём кого-то рядом 👋',
		active: 'Заряжаемся! Где сегодня движ ⚡',
	}
	// выражение Haru под настроение (маппинг подобран по факту):
	// f01 весёлая · f04 улыбка · f06 смущается · f07 унылая · f05 нейтрально
	const EMO: Record<string, number> = {
		social: 1, // Random Coffee → довольная
		romance: 6, // Свидание → смущается
		inspiration: 4, // Вдохновение → улыбка во весь рот
		recharge: 7, // Устал → унылая
		social2: 1, // Найти кого-то → довольная
		active: 4, // Движ → улыбка во весь рот
	}
	const onPick = (mood: string) => {
		setLumiText(REPLIES[mood] ?? 'Секунду…')
		// жест + мимика Live2D под настроение
		window.dispatchEvent(
			new CustomEvent('lumi-react', {
				detail: { motion: 'Tap', expression: EMO[mood] ?? 5 },
			}),
		)
		setTimeout(() => pickMood(mood), 3000)
	}
	...
	return (
		<div style={page}>
			...
			{/* 1. Хедер */}
			<header style={header}>
				<div style={{ ...weatherCard, flex: 1, marginRight: 10 }}>
				...
				{/* 2. Геро — чат с Луми: крупный персонаж → реплика → быстрые ответы */}
				<section style={chatHero}>
					<GuideArt />
					<div style={lumiMsg}>{lumiText}</div>
					<div style={replyRow}>
						{MOODS.map((m) => (
							<button key={m.key} style={replyChip} onClick={() => onPick(m.key)}>
								<m.Icon size={15} color={m.color} strokeWidth={2.2} />
								<span>{m.label}</span>
							</button>
						))}
					</div>
				...
				<div style={{ ...tileGrid, marginBottom: 18 }}>
...
function GuideArt() {
	// без коробки — персонаж стоит на фоне страницы (прозрачный аватар)
	return (
		<div style={artPanel}>
			<GuideAvatar size={252} />
		</div>
	)
}
